...
import { ARBITRUM, AVALANCHE,POLYGON } from "config/chains";
...
export async function getReferralCodeTakenStatus(account, referralCode, chainId) {
  ...
  const [ownerArbitrum, ownerAvax,ownerPolgon] = await Promise.all([
  ...
  const takenOnPolygon =
    !isAddressZero(ownerPolgon) && (ownerPolgon !== account || (ownerPolgon === account && chainId === POLYGON));
