...
import { ARBITRUM, ARBITRUM_TESTNET, AVALANCHE, POLYGON, getConstant, getHighExecutionFee } from "config/chains";
...
export function useMinExecutionFee(library, active, chainId, infoTokens) {
  ...
  // case yet
  if (chainId === ARBITRUM || chainId === ARBITRUM_TESTNET || chainId === POLYGON) {
    multiplier = 65000;
  }
...
export function useStakedGmxSupply(library, active) {
  ...
  const gmxAddressPlogon = getContract(POLYGON, "GMX");
  const stakedGmxTrackerAddressPlogon = getContract(POLYGON, "StakedGmxTracker");

  const { data: plgonData, mutate: plgonMutate } = useSWR(
    [`StakeV2:stakedGmxSupply:${active}`, POLYGON, gmxAddressPlogon, "balanceOf", stakedGmxTrackerAddressPlogon],
    {
      fetcher: contractFetcher(undefined, Token),
    }
  );
  ...
  let data;
  if (arbData && avaxData) {
    data = arbData.add(avaxData).add(plgonData);;
  }
