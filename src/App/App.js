...
import { ARBITRUM, AVALANCHE, POLYGON, getAlchemyWsUrl, getExplorerUrl, POLYGON_RPC_PROVIDERS } from "config/chains";
    ...
    if (!hasMetaMaskWalletExtension()) {
      helperToast.error(
        <div>
          ...
          {userOnMobileDevice ? (
            <Trans>, and use DFEX with its built-in browser</Trans>
          ) : (
            <Trans> to start using DFEX</Trans>
          )}
          .
        </div>
      );
